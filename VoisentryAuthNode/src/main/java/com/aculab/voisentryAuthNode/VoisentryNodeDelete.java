...
/**
 * A node that sends the Delete request to the Voisentry server
 * @author artur.jablonski@aculab.com
 */
@Node.Metadata(outcomeProvider  = VoisentryNodeDelete.VoisentryUpdateNodeOutcomeProvider.class,
               configClass      = VoisentryNodeDelete.Config.class)
public class VoisentryNodeDelete extends AbstractDecisionNode {

    private final Logger logger = LoggerFactory.getLogger(VoisentryNodeDelete.class);
     ...
     */
    @Inject
    public VoisentryNodeDelete(@Assisted Config config, @Assisted Realm realm, AnnotatedServiceRegistry serviceRegistry) throws NodeProcessException {
        this.config = config;
        this.realm = realm;
        try {
    ...
    public Action process(TreeContext context) throws NodeProcessException {
        ...
        logger.info("VoisentryDeleteNode started");
        ...
        if (responseCode == VoisentryResponseCode.ALL_GOOD) {
            ...
            logger.error("Delete failure: Error: " + responseCode);
